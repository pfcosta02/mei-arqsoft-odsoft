...
class IsbnTest {
    ...
    @Test
    void ensureIsbnNotNullException()
    {
    ...
    @Test
    void ensureIsbnMustNotBeBlankException()
    {
    ...
    @Test
    void ensureIsbnMustNotBeOversizeException() 
    {
        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () -> new Isbn("\n" +
                "\n" +
                "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nam venenatis semper nisl, eget condimentum felis tempus vitae. Morbi tempus turpis a felis luctus, ut feugiat tortor mattis. Duis gravida nunc sed augue ultricies tempor. Phasellus ultrices in dolor id viverra. Sed vitae odio ut est vestibulum lacinia sed sed neque. Mauris commodo, leo in tincidunt porta, justo mi commodo arcu, non ultricies ipsum dolor a mauris. Pellentesque convallis vulputate nisl, vel commodo felis ornare nec. Aliquam tristique diam dignissim hendrerit auctor. Mauris nec dolor hendrerit, dignissim urna non, pharetra quam. Sed diam est, convallis nec efficitur eu, sollicitudin ac nibh. In orci leo, dapibus ut eleifend et, suscipit sit amet felis. Integer lectus quam, tristique posuere vulputate sed, tristique eget sem.\n" +
    ...
    @Test
    void ensureChecksum13IsCorrectException() 
    {
        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () -> new Isbn("9782826012099"));
        assertEquals("Invalid ISBN format or check digit.", exception.getMessage());
    }

    @Test
    void ensureChecksum10IsCorrectException() 
    {
        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () -> new Isbn("8175257667"));
        assertEquals("Invalid ISBN format or check digit.", exception.getMessage());
    }

    @Test
    void ensureIsbnInvalidCharactersThrows() 
    {
        assertThrows(IllegalArgumentException.class, () -> new Isbn("978ABC1234567"));
        assertThrows(IllegalArgumentException.class, () -> new Isbn("123456789A"));
    }

    @Test
    void ensureIsbn10WithXLastDigitIsValid() 
    {
        Isbn isbn = new Isbn("048665088X"); // exemplo ISBN10 válido
        assertEquals("048665088X", isbn.toString());
    }

    @Test
    void ensureIsbn13ChecksumZeroIsValid() 
    {
        Isbn isbn = new Isbn("9780306406157"); // ISBN13 válido com checksum 7 (exemplo real)
